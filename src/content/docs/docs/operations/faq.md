...
editUrl: "https://github.com/marmos91/dittofs/edit/develop/docs/guide/faq.md"
sidebar:
  order: 7
# Synced from dittofs/docs/guide/faq.md — do not edit here.
---
...
with no NLM/portmapper setup. See [NFS.md](/docs/connect/nfs#nfsv3-file-locking-nlmnsm).
...
See [ARCHITECTURE.md](/docs/contributing/architecture) for details.
...
[CONFIGURATION.md → BadgerDB cache sizing](/docs/getting-started/configuration#badgerdb-cache-sizing-config-file)
...
and [CONFIGURATION.md](/docs/getting-started/configuration) for the full design and every
`gc.*` knob.
...
See [CLI.md](/docs/getting-started/cli) for the full reference.
...
See [ARCHITECTURE.md](/docs/contributing/architecture#engine-api--blockref--cache)
...
See [NFS.md](/docs/connect/nfs) and [SMB.md](/docs/connect/smb) for more details.
...
macOS restricts SMB mount access to the mount owner regardless of Unix permissions. When using `sudo dfsctl share mount`, it automatically uses `sudo -u $SUDO_USER` to mount as your user. See [SMB.md](/docs/connect/smb) for workarounds.
...
DittoFS is experimental and has not been security audited. See [SECURITY.md](/docs/operations/security) for detailed recommendations.
