---
title: SMB
description: SMB2/3 dialects, encryption, signing, leases, durable handles, and
  client usage.
editUrl: https://github.com/marmos91/dittofs/edit/v0.22.0/docs/guide/smb.md
sidebar:
  order: 2
slug: v0.22/docs/connect/smb
---
...
> coordination live in [../internals/smb-protocol.md](/v0.22/docs/contributing/smb-protocol).
...
| SMB 3.0.2 | 0x0302 | VALIDATE\_NEGOTIATE\_INFO downgrade protection |
...
| SESSION\_SETUP | Implemented | NTLM and Kerberos via SPNEGO, key derivation |
| LOGOFF | Implemented | |
| TREE\_CONNECT | Implemented | Share-level permissions, per-share encryption |
| TREE\_DISCONNECT | Implemented | |
...
| QUERY\_INFO | Implemented | Multiple info classes |
| SET\_INFO | Implemented | Attributes, timestamps, rename, delete |
| QUERY\_DIRECTORY | Implemented | With pagination |
| CHANGE\_NOTIFY | Partial | Accepts watches, async delivery via notification queue |
| IOCTL | Implemented | VALIDATE\_NEGOTIATE\_INFO, FSCTL\_PIPE\_WAIT, server-side copy (SRV\_REQUEST\_RESUME\_KEY + SRV\_COPYCHUNK) |
...
| Negotiate Contexts | Implemented | PREAUTH\_INTEGRITY, ENCRYPTION, SIGNING |
...
| VALIDATE\_NEGOTIATE\_INFO | Implemented | Downgrade protection for 3.0/3.0.2 |
...
| Compound Requests | Implemented | CREATE+QUERY\_INFO+CLOSE |
...
| SACL / auditing | Audit ACEs are not enforced (owner/group/DACL **are** supported — see [Access Control](/v0.22/docs/connect/access-control)) |
...
[./security.md](/v0.22/docs/operations/security#smb3-security-model) for details.
...
See [./configuration.md](/v0.22/docs/getting-started/configuration) for complete encryption configuration options.
See [./security.md](/v0.22/docs/operations/security) for security implications and recommendations.
...
DittoFS validates Kerberos tickets via SPNEGO during SESSION\_SETUP. The SMB adapter shares the
...
See [./security.md](/v0.22/docs/operations/security) for detailed Kerberos security considerations.
...
defaults), see [./configuration.md#user-management](/v0.22/docs/getting-started/configuration#user-management).
...
See [Troubleshooting › Cross-Protocol Issues](/v0.22/docs/operations/troubleshooting#cross-protocol-issues) for cross-protocol troubleshooting, including:
...
7. **No SACL / audit ACEs**: Owner, group, and DACL security descriptors **are** supported (see [Access Control](/v0.22/docs/connect/access-control)); only audit ACEs (SACL) are not enforced
...
| **NT\_STATUS** | Windows error code format |
...
For plain-language definitions of these terms, see the project-wide [Glossary](/v0.22/docs/operations/glossary).
