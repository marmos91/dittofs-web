...
editUrl: https://github.com/marmos91/dittofs/edit/v0.22.0/docs/guide/nfs.md
sidebar:
  order: 1
slug: v0.22/docs/connect/nfs
---

> Implementing or debugging the protocol? See [NFS protocol internals](/v0.22/docs/contributing/nfs-protocol).
...
For the wire format, procedure tables, XDR encoding, and error-code mapping, see [NFS protocol internals](/v0.22/docs/contributing/nfs-protocol).
...
| NFSv4.0 | Stateful, ACLs, compound operations, RPCSEC\_GSS (Kerberos) |
| NFSv4.1 | Sessions, backchannel, directory delegations with CB\_NOTIFY |
| NFSv4.2 | Sparse files (ALLOCATE, DEALLOCATE, SEEK, READ\_PLUS), server-side CLONE/reflink (RFC 7862) + extended attributes (RFC 8276) |

All versions listen on port **12049** by default (not the standard 2049). The embedded portmapper listens on **10111** by default.

CLONE (reflink), ALLOCATE, DEALLOCATE, SEEK, and READ\_PLUS are implemented for NFSv4.2; inter-server COPY (OP\_COPY) is not.
...
* **NLM** (program 100021) versions **1 and 3** (32-bit offsets) and **4**
  ...
  (TEST/LOCK/CANCEL/UNLOCK\_MSG → `*_RES` callbacks) that macOS/BSD `lockd` use,
  alongside the synchronous procedures Linux uses.
* **NSM** (program 100024) version 1, for crash-recovery monitoring (SM\_MON /
  SM\_NOTIFY). The SM\_NOTIFY callback target is the request's transport source,
...
NFSv4.0 and NFSv4.1 can authenticate with Kerberos via RPCSEC\_GSS. The server
...
|--------|---------|--------|
| `allow_auth_sys` | `true` | When `false`, AUTH\_SYS (AUTH\_UNIX) mounts/operations are refused. (Set `require_kerberos` to also refuse AUTH\_NULL and mandate RPCSEC\_GSS — `allow_auth_sys=false` alone only gates AUTH\_SYS.) |
| `require_kerberos` | `false` | When `true`, every mount/operation must use RPCSEC\_GSS; AUTH\_SYS and AUTH\_NULL are refused. |
...
`min_kerberos_level` only constrains RPCSEC\_GSS sessions: it rejects a Kerberos
...
**Principal → identity mapping (the "access denied after EXCHANGE\_ID" case).**
...
See [Troubleshooting](/v0.22/docs/operations/troubleshooting) for common issues. Quick reference:
...
* **`NFS4ERR_WRONGSEC`**: The client used AUTH\_SYS on a Kerberos-only export (or vice versa). Check `allow_auth_sys` / `require_kerberos` on the share.
...
* **Configuration reference**: See [Configuration](/v0.22/docs/getting-started/configuration) for the full `adapters.nfs` config block.
* **Security hardening**: See [Security](/v0.22/docs/operations/security) for TLS, Kerberos, and export policy guidance.
...
| Term | Definition |
|------|------------|
| **AUTH\_NULL** | No authentication flavor (flavor 0) |
| **AUTH\_UNIX** | Unix-style authentication with UID/GID (flavor 1) |
| **Backchannel** | Server-to-client connection used for callbacks (NFSv4.1) |
| **CB\_NOTIFY** | Callback operation for directory change notifications |
...
| **RPCSEC\_GSS** | Kerberos-based RPC security flavor (NFSv4) |
...
For protocol-independent terms (CAS, BLAKE3, SID, ACL, ...) see the project-wide [Glossary](/v0.22/docs/operations/glossary).
...
* [RFC 2203](https://www.rfc-editor.org/rfc/rfc2203) - RPCSEC\_GSS Protocol · [RFC 4120](https://www.rfc-editor.org/rfc/rfc4120) - Kerberos V5 · [RFC 2743](https://www.rfc-editor.org/rfc/rfc2743) - GSS-API
...
* [Architecture](/v0.22/docs/contributing/architecture) - Deep dive into design patterns and implementation
* [Configuration](/v0.22/docs/getting-started/configuration) - Complete configuration guide
* [Glossary](/v0.22/docs/operations/glossary) - Plain-language definitions of protocol, ACL, and storage terms
* [Troubleshooting](/v0.22/docs/operations/troubleshooting) - Common issues and solutions
* [Security](/v0.22/docs/operations/security) - Security hardening guide
* [NFS Protocol Internals](/v0.22/docs/contributing/nfs-protocol) - Wire format, procedure tables, error mapping
