...
editUrl: https://github.com/marmos91/dittofs/edit/v0.22.0/docs/guide/identity.md
sidebar:
  order: 4
slug: v0.22/docs/connect/identity
---
...
* **Kerberos** — accepts Kerberos service tickets for the server's SPN over both
  SMB (SPNEGO) and NFS (RPCSEC\_GSS), from a single service keytab. NTLM remains
  available as a fallback for SMB.

> **Status.** AD/LDAP/Kerberos support is functional but the project is **not
> production ready**. Read [docs/FAQ.md](/v0.22/docs/operations/faq) for known limitations.

See also: [docs/CONFIGURATION.md](/v0.22/docs/getting-started/configuration) (every config key + env var),
[docs/SMB.md](/v0.22/docs/connect/smb), [docs/NFS.md](/v0.22/docs/connect/nfs), and [docs/ACLS.md](/v0.22/docs/connect/access-control) (the
...
|---|---|---|
| **Kerberos** | Authenticates the ticket (SMB SPNEGO / NFS RPCSEC\_GSS), yields a principal + PAC group SIDs | At session/RPC auth time |
| **LDAP idmap** | Maps the principal / SID to a Unix UID/GID + group GIDs | When resolving the authenticated identity to POSIX |

You can enable LDAP alone (idmap for NTLM/AUTH\_UNIX-named principals), Kerberos
...
> issued DC certificate. (See [docs/CONFIGURATION.md §15](/v0.22/docs/getting-started/configuration).)
...
[docs/CONFIGURATION.md §15](/v0.22/docs/getting-started/configuration).
...
Kerberos backs both NFS RPCSEC\_GSS and SMB SPNEGO from a single service keytab.
...
and `DITTOFS_KERBEROS_DNS_DOMAIN` (see [docs/CONFIGURATION.md §12](/v0.22/docs/getting-started/configuration)).
...
`cifs/` principal; NFS RPCSEC\_GSS uses the `nfs/` principal.
...
`local` (standalone behavior). See [docs/SMB.md](/v0.22/docs/connect/smb).
...
* `kerberos.enabled: false` (or omitting the block) renders no `kerberos:` block —
  the server stays AUTH\_UNIX/standalone-SMB only.
...
5. **A running `rpc.gssd`.** Start it cleanly (`systemctl restart rpc-gssd`, now
   ...
   client silently falls back to **AUTH\_SYS `uid=0`**, which a root-squashing
...
* **SID → name resolution** is implemented over the LSARPC pipe: `LsarOpenPolicy`
  ...
  `unix_user:*` / `unix_group:*`). See [docs/ACLS.md](/v0.22/docs/connect/access-control).
