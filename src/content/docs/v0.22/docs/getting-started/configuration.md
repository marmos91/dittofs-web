...
editUrl: https://github.com/marmos91/dittofs/edit/v0.22.0/docs/guide/configuration.md
sidebar:
  order: 3
slug: v0.22/docs/getting-started/configuration
---
...
> [Glossary](/v0.22/docs/operations/glossary) for plain-language definitions.
...
**Recommended deployment model:** terminate TLS for the edge at an ingress / service mesh / reverse proxy (NGINX), and use DittoFS native TLS (or mTLS via `client_ca`) as the secure floor for non-Kubernetes hosts and direct `dfsctl` access. See [docs/SECURITY.md](/v0.22/docs/operations/security) and [docs/DEPLOYMENT.md](/v0.22/docs/getting-started/install). For Kubernetes, the operator renders `host: 0.0.0.0` automatically so the API `Service` can reach the pod; see [docs/DEPLOYMENT.md](/v0.22/docs/getting-started/install).

Related glossary terms: [TLS / mTLS](/v0.22/docs/operations/glossary#authentication).
...
for the full mark-sweep design and [CLI.md](/v0.22/docs/getting-started/cli) for the on-demand
...
See [ENCRYPTION.md](/v0.22/docs/operations/encryption) for the full threat model and design.
...
`dfsctl` against a running server (run `dfsctl login` first). See [CLI.md](/v0.22/docs/getting-started/cli) for the
...
explicit override); NFS RPCSEC\_GSS uses the `nfs/` principal. Online
...
[SNAPSHOTS.md §12](/v0.22/docs/operations/snapshots#12-scheduled-snapshots-policies). These
...
(see [CLI.md](/v0.22/docs/getting-started/cli)).
...
* [docs/CLI.md — `dfs migrate-to-cas`](/v0.22/docs/getting-started/cli#dfs-migrate-to-cas) for the
