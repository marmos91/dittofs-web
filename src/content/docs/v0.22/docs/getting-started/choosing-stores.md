...
editUrl: https://github.com/marmos91/dittofs/edit/v0.22.0/docs/guide/choosing-stores.md
sidebar:
  order: 4
slug: v0.22/docs/getting-started/choosing-stores
---
...
the exact config keys and CLI flags, see [Configuration](/v0.22/docs/getting-started/configuration).
...
* DittoFS speaks the **S3 API**, so [Cubbit DS3](https://www.cubbit.io/) (a DittoFS sponsor),
  ...
  see the verified endpoint snippets in [Configuration § Block Store](/v0.22/docs/getting-started/configuration#6-block-store-configuration).
...
* Tune append-log pressure (`max_log_bytes`, default ~25% of capacity) and durability
  (`require_durable_commit`) per store — see [Configuration](/v0.22/docs/getting-started/configuration).
* To migrate a legacy block layout to the content-addressed layout, see
  [Block store migration](/v0.22/docs/operations/block-store-migration).
...
[Implementing stores](/v0.22/docs/contributing/implementing-stores).
