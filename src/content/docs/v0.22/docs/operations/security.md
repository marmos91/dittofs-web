...
editUrl: https://github.com/marmos91/dittofs/edit/v0.22.0/docs/guide/security.md
sidebar:
  order: 4
slug: v0.22/docs/operations/security
---
...
[`../internals/security-model.md`](/v0.22/docs/contributing/security-model).
...
* Kerberos authentication for NFS via RPCSEC\_GSS (RFC 2203)
...
* VALIDATE\_NEGOTIATE\_INFO for SMB 3.0/3.0.2 downgrade detection
...
* AUTH\_UNIX for trusted-network deployments
...
### NFS: RPCSEC\_GSS (Kerberos)

DittoFS implements RPCSEC\_GSS per RFC 2203, enabling Kerberos-based strong authentication for
...
AUTH\_UNIX is the traditional NFS authentication mechanism. The client provides UID, GID, and
...
AUTH\_NULL provides anonymous access with no authentication. All requests are treated as coming
...
See [`../internals/security-model.md`](/v0.22/docs/contributing/security-model) for cipher/key derivation
...
See [`./configuration.md`](/v0.22/docs/getting-started/configuration) for the full option reference.
...
DittoFS uses a shared Kerberos layer (`pkg/auth/kerberos`) that serves both the NFS (RPCSEC\_GSS)
...
| `context_ttl` | Maximum lifetime of an RPCSEC\_GSS security context | `8h` |
...
See [`./configuration.md`](/v0.22/docs/getting-started/configuration) for the full adapter configuration reference.
